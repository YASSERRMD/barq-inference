...
/// ```no_run
/// use std::sync::Arc;
/// use models::{loader::Model, Qwen2VlModel};
///
/// # async fn demo() -> Result<(), Box<dyn std::error::Error>> {
/// let model = Arc::new(Model::load("qwen2vl.gguf").await?);
/// let wrapper = Qwen2VlModel::new(model)?;
/// let prompt = wrapper.insert_image_tokens("describe this", 1);
...
impl Qwen2VlModel {
    /// Create a Qwen2-VL wrapper around a loaded model.
