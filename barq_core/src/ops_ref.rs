...
pub fn add_ref(lhs: &Tensor, rhs: &Tensor) -> Result<Tensor> {
    ...
    match lhs.dtype() {
        TensorType::F32 => {
            ...
            let result: Vec<f32> = lhs_data.iter().zip(rhs_data.iter()).map(|(a, b)| a + b).collect();
            Tensor::new(None, TensorType::F32, lhs.shape().clone(), crate::tensor::TensorData::F32(result))
        }
        _ => Err(Error::Unsupported(format!("Add not implemented for {}", lhs.dtype()))),
    }
}
...
pub fn matmul_ref(lhs: &Tensor, rhs: &Tensor) -> Result<Tensor> {
    ...
    match (lhs.dtype(), rhs.dtype()) {
        (TensorType::F32, TensorType::F32) => {
            ...
            Tensor::new(None, TensorType::F32, Shape::matrix(m, n), crate::tensor::TensorData::F32(result))
        }
        _ => Err(Error::Unsupported(format!("MatMul not implemented for {} x {}", lhs.dtype(), rhs.dtype()))),
    }
}
